    ...
    <div className="grid gap-6">
      ...
      >
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[320px_1fr]">
          <div className="rounded-[24px] bg-slate-50 p-4">
            <p className="text-sm font-semibold text-slate-900">Conference content</p>
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      isActive ? "bg-ink text-white shadow-panel" : "bg-white text-slate-700 hover:bg-slate-100",
                    ].join(" ")}
                  >
          ...
          <div className="rounded-[24px] bg-slate-50 p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-sm uppercase tracking-[0.2em] text-slate-400">Selected section</p>
                <h3 className="mt-2 text-2xl font-semibold text-slate-950">{selectedSection?.label || "Conference"}</h3>
                <p className="mt-1 text-sm text-slate-500">
                  {activeConference?.name ? `Active: ${activeConference.name}` : activeConferenceId ? `Active ID: ${activeConferenceId}` : "Select a conference above."}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5">
              {selectedSection?.key === "important-dates" ? (
                <div className="space-y-4">
                  <div className="rounded-[24px] bg-white p-5">
                    <p className="text-sm uppercase tracking-[0.2em] text-slate-400">Important dates</p>
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Submission deadline: May 14, 2026</div>
                      <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Review deadline: June 05, 2026</div>
                      <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Notification: June 14, 2026</div>
                      <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Conference start: July 09, 2026</div>
                    </div>
                  </div>

                  <div className="space-y-3">
                    {tracks.map((track) => (
                      <div key={track.name} className="flex items-center justify-between gap-3 rounded-2xl border border-white/70 bg-white p-4">
                        <div>
                          <p className="font-semibold text-slate-900">{track.name}</p>
                          <p className="text-sm text-slate-500">{track.chair}</p>
                        </div>
                        <span className="rounded-full bg-slate-100 px-3 py-2 text-sm text-slate-700">{track.submissions} papers</span>
                      </div>
                    ))}
                  </div>
                </div>
              ) : selectedSection?.key === "about-events" ? (
                <div className="space-y-4">
                  {aboutEventSuccess ? (
                  ...
                  {aboutEventError ? (
                    <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                      <p>{aboutEventError}</p>
                      <a
                        href={getAdminAboutEventListUrl()}
                        target="_blank"
                        rel="noreferrer"
                        className="mt-3 inline-flex items-center gap-2 rounded-full border border-rose-200 bg-white px-3 py-1.5 text-xs font-semibold text-rose-700"
                      >
                        Open Django admin list
                      </a>
                    </div>
                  ) : null}

                  {!activeConferenceId ? (
                    <div className="rounded-[24px] bg-white p-6 text-sm text-slate-600">
                      <p className="font-semibold text-slate-900">No active conference selected</p>
                      <p className="mt-2">Pick a conference from the list above, then manage About Event content.</p>
                    </div>
                  ) : isLoadingAboutEvents ? (
                    <div className="rounded-[24px] bg-white p-6 text-sm text-slate-600">Loading...</div>
                  ) : (
                    <>
                      <div className="flex flex-wrap items-center justify-between gap-3 rounded-[24px] bg-white p-4">
                        <div>
                          <p className="text-sm font-semibold text-slate-900">About event entries</p>
                          <p className="mt-1 text-xs text-slate-500">Conference ID: {activeConferenceId}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => loadAboutEvents(aboutEventPage)}
                            className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700"
                          >
                            Refresh
                          </button>
                      ...
                      <div className="space-y-3">
                        {aboutEvents.length ? (
                          aboutEvents.map((item) => (
                            <div key={item.id || item.hero_title || JSON.stringify(item)} className="rounded-[24px] bg-white p-5">
                              <div className="flex flex-wrap items-start justify-between gap-3">
                                <div className="min-w-0">
                                  <p className="truncate text-sm font-semibold text-slate-900">{item.hero_title || "About event"}</p>
                                  <p className="mt-1 line-clamp-2 text-sm text-slate-600">{item.hero_summary || "—"}</p>
                                </div>
                                <div className="flex items-center gap-2">
                                  <button
                                    type="button"
                                    onClick={() => openEditAboutEvent(item)}
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => handleDeleteAboutEvent(item)}
                                    className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-white px-3 py-2 text-xs font-semibold text-rose-700"
                                    disabled={!item?.id}
                                  >
                                    <Trash2 size={14} />
                                    Delete
                              ...
                              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                                <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">
                                  ...
                                  <p className="mt-2 font-semibold text-slate-900">{item.hero_badge || "—"}</p>
                                </div>
                                <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">
                                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Contact email</p>
                                  <p className="mt-2 font-semibold text-slate-900">{item.contact?.email || "—"}</p>
                                </div>
                              </div>
                            </div>
                          ))
                          ...
                          <div className="rounded-[24px] bg-white p-6 text-sm text-slate-600">
                            <p className="font-semibold text-slate-900">No about event content yet</p>
                            <p className="mt-2">Add an entry to populate the public About Event section.</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-2 pt-1">
                        <button
                          type="button"
                          disabled={!aboutEventPagination.previous || isLoadingAboutEvents}
                          onClick={() => loadAboutEvents(Math.max(1, aboutEventPage - 1))}
                          className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          Prev
                        </button>
                        <button
                          type="button"
                          disabled={!aboutEventPagination.next || isLoadingAboutEvents}
                          onClick={() => loadAboutEvents(aboutEventPage + 1)}
                          className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          Next
                        </button>
              ...
              ) : selectedSection?.key === "submission-configs" ? (
                <div className="space-y-3">
                  {tracks.map((track) => (
                    <div key={track.name} className="flex items-center justify-between gap-3 rounded-2xl border border-white/70 bg-white p-4">
                      <div>
                ...
                <div className="rounded-[24px] bg-white p-6 text-sm text-slate-600">
                  <p className="font-semibold text-slate-900">Nothing added yet</p>
                  <p className="mt-2">Use the action button to start populating this section.</p>
                </div>
              )}
            </div>
          </div>
